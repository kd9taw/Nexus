...
for (const operationVersion of [1, 2, 3]) test(`actual cloud and native operations v${operationVersion} produce one durable QSO, preserve receipts and refuse local takeover`, {timeout:60000},async()=>{
 ...
 try{
  ...
  if(operationVersion>=2){
   ...
   assert.deepEqual(controls.controls.capabilities,operationVersion===3?['decoder','amplifier','frequency','mode','tier','ampFollowBand','workspace','decoderSettings','receiverSettings','receiverGain','bandSelection','receiverFilter','receiverDsp','phoneMode','workSpot','radioLevels','radioSelection','fmTuning']:['decoder','amplifier'])
