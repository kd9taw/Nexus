...
test('expanded operations preserve legacy clients, minimum peer versions and hibernating command routes', async () => {
  ...
  for (const stationVersion of [1, 2, 3]) for (const browserVersion of [1, 2, 3]) {
    ...
    const state = { stationBootId: crypto.randomUUID(), allowed: true, phase: 'controlling', leaseId: crypto.randomUUID(), revision: 1,
      ...
      ...(negotiated >= 2 ? { controls: { context: { radioId: 1, radioConnection: 1, ampConnection: null, ampReadSequence: null }, capabilities: negotiated === 3 ? ['decoder', 'amplifier', 'frequency', 'mode', 'tier', 'ampFollowBand', 'workspace', 'decoderSettings','receiverSettings','receiverGain','bandSelection','receiverFilter'] : ['decoder', 'amplifier'] } } : {}) }
    await app.evict(pair.stationId)
    // Simulate a station predating v3 that attached expanded hints under v2.
    const wire = structuredClone(state)
    if (negotiated >= 2) wire.controls.capabilities = ['decoder', 'amplifier', 'frequency', 'mode', 'tier', 'ampFollowBand', 'workspace', 'decoderSettings','receiverSettings','receiverGain','bandSelection','receiverFilter']
