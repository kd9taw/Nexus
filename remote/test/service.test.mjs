...
test('expanded operations preserve legacy clients, minimum peer versions and hibernating command routes', async () => {
  ...
  for (const stationVersion of [1, 2, 3]) for (const browserVersion of [1, 2, 3]) {
    ...
    const state = { stationBootId: crypto.randomUUID(), allowed: true, phase: 'controlling', leaseId: crypto.randomUUID(), revision: 1,
      ...
      ...(negotiated >= 2 ? { controls: { context: { radioId: 1, radioConnection: 1, ampConnection: null, ampReadSequence: null }, capabilities: negotiated === 3 ? ['decoder', 'amplifier', 'frequency', 'mode', 'tier', 'ampFollowBand', 'workspace', 'decoderSettings'] : ['decoder', 'amplifier'] } } : {}) }
    await app.evict(pair.stationId)
    // Simulate a station predating v3 that attached expanded hints under v2.
    const wire = structuredClone(state)
    if (negotiated >= 2) wire.controls.capabilities = ['decoder', 'amplifier', 'frequency', 'mode', 'tier', 'ampFollowBand', 'workspace', 'decoderSettings']
    ...
    if (browserVersion >= 2) {
      for (const action of [{ action: 'radio.tier', tier: 'FT4' }, { action: 'radio.workspace', workspace: 'js8' }, { action: 'amplifier.followBand', radioId: 1, expectedSettingsRevision: 'a'.repeat(64), expectedFollow: true, follow: false }, { action: 'decoder.js8Speed', expectedSpeed: 1, speed: 3 }, { action: 'decoder.msk144Period', expectedPeriodSecs: 15, periodSecs: 5 }, { action: 'decoder.clear', receiver: 'cw' }]) {
        await delay(270)
        ...
        if (negotiated < (action.action === 'decoder.clear' ? 2 : 3)) {
          ...
          live.station.send({ type: 'operationResponse', sessionId: routed.sessionId, requestId: command.requestId,
            value: { operation: 'stationControl', operationId: command.requestId, outcome: 'applied', evidence: ['amplifier.followBand','decoder.js8Speed','decoder.msk144Period'].includes(action.action) ? 'settingsSaved' : action.action.startsWith('radio.') ? 'radioReadback' : 'receiverState' } })
