...
const APPLICATION_VERSIONS = (await readFile(new URL('../../ui/src/remote-web/application-capabilities.ts', import.meta.url), 'utf8'))
...
for (const {applicationVersion,operating,sessionLayout,quickLayout,quickMode='phone',contactContinuity,workSpot,radioSelection,routedTier,routedWorkspace,ftOperating} of SHARD_SCENARIOS) test(`compiled hosted browser ${ftOperating?'FT operating':routedWorkspace?'routed workspace':routedTier?'routed decoder':radioSelection?'radio selection':workSpot?'DX work':contactContinuity?'contact continuity':quickLayout?`quick layout${quickMode==='cw'?' CW':''}`:sessionLayout?'session layout':operating?'operations':`v${applicationVersion}`} completes PKCE, local device approval, observation and viewport checks`, { timeout: applicationVersion >= 14 ? 540000 : applicationVersion >= 13 ? 420000 : 180000 }, async context => {
  ...
  try {
    ...
    await browser.call('Page.addScriptToEvaluateOnNewDocument',{source:`if(window===window.top){
      ...
      window.__queryTrace=[];window.__availabilityTrace=[];window.__socketClosures=[];window.__protocolTrace=[];const originalWebSocket=window.WebSocket;
      window.WebSocket=class extends originalWebSocket{
        constructor(...args){super(...args);this.addEventListener('close',e=>window.__socketClosures.push({at:performance.now(),code:e.code,reason:e.reason}));this.addEventListener('message',e=>{try{const v=JSON.parse(e.data);if(['applicationQueryError','applicationPage'].includes(v.type)){const m=v.meta?.source;window.__queryTrace.push({at:performance.now(),type:v.type,id:v.requestId,error:v.error,collection:v.collection,age:v.ageMs,document:{kind:m?.kind,search:m?.search,id:m?.contextId,context:m?.stationContextId,validForMs:m?.validForMs,ageMs:m?.documentAgeMs,capturedAgeMs:v.meta?.capturedAgeMs}});window.__queryTrace=window.__queryTrace.slice(-80)}if(v.type==='applicationFrame'){window.__protocolTrace.push({at:performance.now(),updates:v.updates.map(u=>({command:u.command,type:u.type,age:u.ageMs,revision:u.revision}))});window.__protocolTrace=window.__protocolTrace.slice(-40)}}catch{}})}
        send(raw){try{const v=JSON.parse(raw);if(v.type==='applicationQuery'){window.__queryTrace.push({at:performance.now(),out:true,id:v.requestId,collection:v.collection,search:v.search,cursor:v.cursor});window.__queryTrace=window.__queryTrace.slice(-80)}window.__protocolTrace.push({at:performance.now(),out:v.type,requestId:v.requestId,topics:v.topics,buffered:this.bufferedAmount});window.__protocolTrace=window.__protocolTrace.slice(-80)}catch{}return super.send(raw)}
      ...
      if(request.type==='operationRequest'){
        ...
        if(r.type==='stopTransmit'){
          ...
          if(a.action==='ft.setting'){
            ...
            assert.equal(a.expectedTier,applicationData.get_snapshot.link.tier)
            const q=applicationData.get_snapshot.qso
            assert.deepEqual(a.expectedQso,{dxcall:q.dxcall,state:q.state,txNow:q.txNow,cqRunning:q.cqRunning})
            if(qsoPrompt){applicationData.get_snapshot.pendingLog={...qsoRecord};applicationData.get_snapshot.pendingQsoLogKey='0000000000000002'}
            qsoEvidence=qsoPrompt?'pendingConfirmationSynced':'fileSynced'
          }else if(a.action==='qso.confirm'||a.action==='qso.discard'){
            assert.ok(loggingAllowed)
            assert.equal(a.expectedKey,applicationData.get_snapshot.pendingQsoLogKey)
            if(a.action==='qso.confirm')assert.deepEqual(a.edits,{call:'W1AW',grid:'FN32',rstSent:'-10',rstRcvd:'-12'})
            applicationData.get_snapshot.pendingLog=null;applicationData.get_snapshot.pendingQsoLogKey=null
            qsoEvidence=a.action==='qso.confirm'?'fileSynced':'pendingDiscarded'
          }else if(a.action==='ft.message'){
            assert.ok(ftOperating&&transmitAllowed);assert.equal(a.transmitEpoch,transmitEpoch)
            assert.equal(a.expectedTier,applicationData.get_snapshot.link.tier)
            ...
            assert.equal(a.call,'K2ABC');assert.equal(a.grid,'FN42')
            Object.assign(qso,{dxcall:a.call,dxgrid:a.grid,state:'awaitReport',txNow:a.text,cqRunning:false,running:true})
          }else if(a.action==='ft.exchange'){
            assert.ok(ftOperating&&transmitAllowed);assert.equal(a.transmitEpoch,transmitEpoch)
            assert.equal(a.expectedTier,applicationData.get_snapshot.link.tier)
            const qso=applicationData.get_snapshot.qso
            assert.deepEqual(a.expectedQso,{dxcall:qso.dxcall,state:qso.state,txNow:qso.txNow,cqRunning:qso.cqRunning})
            ...
            applicationData.get_snapshot.radio.txEnabled=true
            applicationData.get_snapshot.qso={state:'awaitReport',dxcall:'W1AW',dxgrid:'FN31',rxReport:null,running:true,cqRunning:false,txNow:'W1AW N0CALL AA00'}
            ...
            assert.ok(['auto','USB','LSB','FM','AM'].includes(a.mode));assert.equal(radio.txEnabled,false);assert.equal(radio.rigKeyed,false)
            ...
            if(routedTier){
              ...
              else assert.equal(a.tier,'FT8') // no FT8 channel on 2200m: keep the dial/radio
            }
    ...
    if(ftOperating){
      ...
      const operatorGesture=async(label,target,done)=>{
        for(let attempt=1;;attempt++){
          ...
          if(stationRequests.length!==before)await txEvidence(`${label} ${outcome}`)
          assert.equal(stationRequests.length,before,`${label}: a ${outcome} must not reach the station`)
          assert.ok(attempt<5,`${label}: ${outcome} ${attempt} times in a row`)
          console.log('FT operator retry',label,outcome,attempt)
        }
      }
      ...
      for(const tier of ['FT8','FT4']){
        ...
        await operatorGesture('cq',`document.querySelector('${cq}')`,`document.querySelector('${tx}')?.textContent.trim()==='TX On'&&!document.querySelector('${tx}').disabled`)
        ...
        count+=4
        await operatorGesture('tx-off',`document.querySelector('${tx}')`,`document.querySelector('${tx}')?.textContent.trim()==='TX Off'&&!document.querySelector('${tx}').disabled`)
        assert.equal(stationRequests.length,count+2);assert.equal(stationRequests.at(-1).action.on,false)
        await operatorGesture('tx-on',`document.querySelector('${tx}')`,`document.querySelector('${tx}')?.textContent.trim()==='TX On'&&!document.querySelector('${tx}').disabled`)
        ...
        await click(`document.querySelectorAll('.cockpit-layout-toggle .clt-opt')[0]`)
        await until(`!!document.querySelector('.tx-panel')`)
        for(const [index,value]of [[0,'K2ABC'],[1,'FN42']]){
          const field=`document.querySelectorAll('.txp-dx input')[${index}]`
          await click(field);await evaluate(`${field}.select()`);await browser.call('Input.insertText',{text:value},session)
        }
        for(let n=1;n<=6;n++){
          const target=`document.querySelectorAll('.txp-btn')[${n-1}]`
      ...
      await operatorGesture('log-confirm',`document.querySelector('${log}')`,`!!document.querySelector('.logconfirm')&&!document.querySelector('${confirm}').disabled`)
      ...
      assert.equal(await evaluate(`document.querySelector('${discard}').disabled`),true)
      await click(`document.querySelector('.logconfirm [data-remote-stop]')`)
      for(let i=0;i<100&&stopRequests.length<3;i++)await sleep(50)
      assert.equal(stopRequests.length,3)
      unavailableTopics.delete('get_snapshot');applicationRevision++
      await until(`!document.querySelector('${confirm}').disabled`)
    ...
    if(radioSelection){
      ...
      if(routedTier){
        ...
        for(const [selector,tier,id,mhz] of [
          ['.topbar-group.tier-toggle:not(.tx-period) > button:nth-child(8)','MSK144',2,50.260],
          ['.topbar-group.tier-toggle:not(.tx-period) > button:nth-child(10)','FST4',1,0.136],
          ['.operate-cockpit .cockpit-modes > button:first-child','FT8',1,0.136],
        ]){
          ...
          assert.deepEqual(stationRequests.at(-1).context,context)
          assert.equal(applicationData.get_snapshot.radio.dialMhz,mhz)
          assert.equal(applicationData.get_snapshot.radio.txEnabled,false)
        }
        stationControls=false
        await until(`document.querySelector('.topbar-group.tier-toggle:not(.tx-period) > button:nth-child(8)')?.disabled===true`)
        assert.equal(stationRequests.length,3);assert.equal(exceptions,0);assert.equal(unexpectedMessages,0)
        if(artifacts){
          const shot=await browser.call('Page.captureScreenshot',{format:'png'},session)
          await writeFile(join(artifacts,'routed-decoder.png'),Buffer.from(shot.data,'base64'))
          await writeFile(join(artifacts,'routed-decoder-results.json'),JSON.stringify({actions:stationRequests.map(r=>({action:r.action,context:r.context})),selectionGeometry,missingChannelRetained:true,revocationRefusal:true,exceptions,unexpectedMessages},null,2))
        }
        console.log('Compiled routed decoder: native selectors, station radio context, missing-channel retention and revocation passed');return
      }
    ...
    if(operating){
      ...
      for(const [tab,root,mode,workspace] of [['CW','.cw-cockpit','cw'],['FT','.operate-cockpit','digital','ft'],['Phone','.phone-cockpit','phone'],['RTTY','.rtty-cockpit','rtty'],['PSK','.psk-cockpit','keyboard'],['Tempo','.grid-header','digital','tempo'],['JS8','.js8-cockpit','digital','js8'],['FT','.operate-cockpit','digital','ft']]){
        ...
        if(mode==='phone'){
          ...
          // The following five picks are the fresh positive control.
          ...
          for(const [pick,index]of [['USB',2],['LSB',3],['AM',5],['FM',4],['auto',1]]){
        ...
        if(['cw','phone'].includes(mode)){
          ...
          // ⭐ THE SECOND `.ph-dsp` SITE, and the assertion gets STRONGER rather than looser.
          // Positions 4 and 5 were COMP and VOX in the old combined pane, asserted to exist and
          // be DISABLED because this station reports neither and `canFunction` was false. The
          // 2026-09-20 rebuild resolves that same state to ABSENT — `causeFor` returns 'absent'
          // for a control this radio has never reported and `rendersRow` drops the row, so a
          // rig that lacks four things does not open to four grey rows. That is the ruling's
          // second recorded exception, not a control going quiet: the plate goes in the pane's
          // foot line instead. So both halves are asserted — nothing DRIVABLE, and the operator
          // can still find out why. A bare `?.disabled` check would have read `undefined` here
