...
for (const model of uniq) {
  ...
  if (!speed) {
    // WAS A BARE `continue`, AND THAT COST US A RIG. A model that dumps caps but declares no
    // `Serial speed:` line landed in NO list — not rigs, not missing, not needsDaemon — so it
    // was invisible to every guard downstream. Five backends belong here legitimately (Dummy,
    // FLRig, TRX-Manager, the two Flex entries); the FT-890 did not, and because the fixture is
    // only regenerated by hand it stayed absent while `missing: []` read as full coverage. It is
    // a one-rate 4800 radio, so the guard that exists to stop exactly the FX-4 bug could not see
    // the one rig it needed to. Recording the fallthrough is what makes the next one loud.
    noSerial.push(model)
    continue
  }
...
writeFileSync(
  out,
  JSON.stringify({ hamlib, catalog: uniq.length, rigs, missing, needsDaemon, noSerial }, null, 2) + '\n',
)
...
if (noSerial.length) console.log(`no Serial speed line (must all be serial-less backends): ${noSerial.join(' ')}`)
