...
const command = 'get_remote_memories'
const args = { collection: 'memories', cursor: null, search: '', unconfirmed: false, after: null }
const request = () => ({ ...args, type: 'applicationQuery', requestId: crypto.randomUUID() })

it('negotiates Memories without broadening the seven older contracts', async () => {
  for (let stationVersion = 1; stationVersion <= 8; stationVersion++) for (let browserVersion = 1; browserVersion <= 8; browserVersion++) {
    ...
    expect(client.supports(command)).toBe(stationVersion === 8 && browserVersion === 8)
    if (stationVersion < 8 || browserVersion < 8) {
...
it('admits only an argument-free Memories read on its explicit command', async () => {
  expect(applicationCommands(8)).toContain(command)
  for (const version of [3, 4, 5, 6, 7]) expect(() => queryRequest(request(), version)).toThrow()
  expect(queryRequest(request(), 8).collection).toBe('memories')
  for (const patch of [{ search: 'W1AW' }, { unconfirmed: true }, { after: 1 }, { cursor: `${crypto.randomUUID()}:1` }, { path: 'log.adi' }, { bank: {} }, { generation: '1' }]) {
    expect(() => queryRequest({ ...request(), ...patch }, 8)).toThrow()
  }
  const station = peer(), browser = peer(), relay = new ApplicationRelay()
  relay.sync({ peer: station, version: 8 }, [{ sessionId: 'one', peer: browser }], 0)
  const client = new ApplicationClient(s => relay.receiveBrowser('one', JSON.parse(s), 0), vi.fn(), 8)
  client.open(); client.receive(last(browser))
  for (const old of ['get_remote_page', 'get_remote_recall', 'get_remote_insights', 'get_remote_dxpeditions', 'publish_remote_memory_bank']) await expect(client.invoke(old, args)).rejects.toThrow()
  ...
  expect(last(station).collection).toBe('memories')
  client.disconnected(); await expect(pending).rejects.toThrow('applicationUnavailable')
})

it('resumes a query after hibernation without persisting memory data or granting it to v7', () => {
  ...
  relay.sync({ peer: station, version: 8 }, peers, 0)
  relay.receiveBrowser('modern', { type: 'applicationHello', version: 8 }, 0)
  relay.receiveBrowser('old', { type: 'applicationHello', version: 7 }, 0)
  const read = request(); relay.receiveBrowser('modern', read, 1)
  const forwarded = last(station), saved = relay.checkpoint('modern')!
  expect(saved.version).toBe(8)
  const resumed = new ApplicationRelay(); resumed.sync({ peer: station, version: 8 }, peers, 2); resumed.restore('modern', saved)
  resumed.receiveStation({ type: 'applicationPage', requestId: forwarded.requestId, collection: 'memories', snapshotId: crypto.randomUUID(),
