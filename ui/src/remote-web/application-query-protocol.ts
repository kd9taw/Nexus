...
export type Collection = typeof COLLECTIONS[number] | 'recall' | InsightCollection | 'dxpeditions' | 'memories' | 'ota'
...
export const collection = (v: unknown, version = 3): v is Collection => COLLECTIONS.includes(v as typeof COLLECTIONS[number]) ||
  ([4, 6, 7, 8, 9].includes(version) && v === 'recall') || ([6, 7, 8, 9].includes(version) && insightCollection(v)) || ([7, 8, 9].includes(version) && v === 'dxpeditions') || ([8, 9].includes(version) && v === 'memories') || (version === 9 && v === 'ota')
...
export function queryRequest(v: Record<string, unknown>, version = 3): QueryRequest {
  ...
  if (v.type !== 'applicationQuery' || !streamId(v.requestId) || !collection(v.collection, version) ||
    ...
    ((insightCollection(v.collection) || v.collection === 'dxpeditions' || v.collection === 'memories' || v.collection === 'ota') && v.cursor !== null) ||
