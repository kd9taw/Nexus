...
export type Collection = typeof COLLECTIONS[number] | 'recall' | InsightCollection | 'dxpeditions' | 'memories' | 'ota' | 'fieldDay' | 'js8Context' | 'sstvImage' | 'aprs' | DocumentCollection | 'parks'
...
export function queryRequest(v: Record<string, unknown>, version = 3): QueryRequest {
  ...
  if (v.type !== 'applicationQuery' || !streamId(v.requestId) || !collection(v.collection, version) ||
    ...
    ((insightCollection(v.collection) || v.collection === 'parks' || v.collection === 'dxpeditions' || v.collection === 'memories' || v.collection === 'ota' || v.collection === 'fieldDay' || v.collection === 'js8Context') && v.cursor !== null) ||
