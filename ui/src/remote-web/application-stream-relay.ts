...
import { streamVocabulary, streamExact, streamId, streamTopics, streamUpdates } from './application-stream-protocol'
import type { StreamTopic, StreamUpdate, StreamError, StreamVersion } from './application-stream-protocol'
import type { Peer } from '../remote-monitor/relay'

type Credit = { id: string; at: number }
type Offered = { serial: number; revision?: number }
export type StreamCheckpoint = { version: StreamVersion; topics: StreamTopic[]; credit: Credit | null; pending: Credit | null; windowAt: number; count: number }
...
export class ApplicationStreamRelay {
  ...
  sync(station: Peer | null, peers: { sessionId: string; peer: Peer }[], now: number, version: StreamVersion = 2): void {
    if (this.station !== station || this.version !== version) {
  ...
  add(sessionId: string, now: number, version: StreamVersion = 2): void {
    const peer = this.peers.get(sessionId)
    if (!peer || !this.station || this.has(sessionId) || version > this.version) throw new Error('invalidApplicationSession')
    this.browsers.set(sessionId, { version, peer, topics: [], credit: null, pending: null, windowAt: now, count: 0,
      revisions: new Map(), seen: new Map(), offered: new Map() })
  }
  restore(sessionId: string, saved: StreamCheckpoint): void {
    const peer = this.peers.get(sessionId)
    if (!peer || !this.station || ![2, 5, 11, 12, 13].includes(saved.version) || saved.version > this.version) throw new Error('invalidApplicationCheckpoint')
  ...
  checkpoint(sessionId: string): StreamCheckpoint | undefined {
    ...
    if (b) return { version: b.version, topics: b.topics, credit: b.credit, pending: b.pending, windowAt: b.windowAt, count: b.count }
  }
  receiveBrowser(sessionId: string, message: Record<string, unknown>, now: number): void {
    ...
    try {
      ...
      if (message.type === 'applicationSubscribe') {
        ...
        const topics = streamTopics(message.topics, b.version)
  ...
  private refresh(now: number): void {
    if (!this.station) return
    const topics = streamVocabulary(this.version).filter(topic => [...this.browsers.values()].some(b => b.topics.includes(topic)))
