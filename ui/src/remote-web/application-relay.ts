...
export type ApplicationCheckpoint = LegacyCheckpoint | (StreamCheckpoint & { version: 2 }) | { version: 3 | 4 | 5 | 6; stream: StreamCheckpoint; query: QueryCheckpoint }
...
export class ApplicationRelay {
  ...
  restore(sessionId: string, saved: ApplicationCheckpoint): void {
    if (saved.version === 3 || saved.version === 4 || saved.version === 5 || saved.version === 6) {
  ...
  checkpoint(sessionId: string): ApplicationCheckpoint | undefined {
    ...
    if (stream && query) return { version: query.version === 6 ? 6 : stream.version === 5 ? 5 : query.version ?? 3, stream, query }
