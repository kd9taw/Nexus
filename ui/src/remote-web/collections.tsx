  ...
  async page(args: QueryArgs): Promise<QueryPage> {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const page = await this.client.invoke<QueryPage>(args.collection === 'js8Context' ? JS8_CONTEXT_COMMAND : args.collection === 'fieldDay' ? FIELD_DAY_COMMAND : args.collection === 'ota' ? OTA_COMMAND : args.collection === 'memories' ? MEMORIES_COMMAND : args.collection === 'dxpeditions' ? DXPEDITIONS_COMMAND : args.collection === 'recall' ? RECALL_COMMAND : insightCollection(args.collection) ? INSIGHTS_COMMAND : QUERY_COMMAND, args)
        if (!this.live || generation !== this.generation) throw new Error('applicationUnavailable')
        return page
      }
      catch (error) {
