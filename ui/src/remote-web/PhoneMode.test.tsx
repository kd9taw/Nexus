...
it('keeps FM unavailable and offers AM on every band, as the native cockpit does', async () => {
  // AM used to be hidden below 10 MHz and at 28 MHz and up, on the reasoning that
  // "20/17/15/12 m are not AM territory". That was wrong on the one frequency it most
  // needed to be right about — 14.286 is the 20 m AM calling frequency — and an operator
  // working it there found no AM button, set AM at the radio, and had the contact logged
  // as SSB. The filter went on 2026-09-10; USB, LSB and FM never had one either.
  const h = fixture(); await tick()
  ...
  expect(h.button('AM').disabled).toBe(false); expect(h.button('USB').disabled).toBe(false)
