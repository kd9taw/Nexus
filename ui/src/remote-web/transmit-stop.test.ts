...
it('routes Stop despite ordinary rate exhaustion and pending requests, retaining all routes across hibernation', () => {
  ...
  send(stop(s)); expect(browser.frames.at(-1).error).toBe('remoteBusy')
  ...
  expect(browser.frames.at(-1)).toEqual({ type: 'operationResponse', requestId: request.requestId, value: { stop: 'accepted' } })
...
it('sends Stop while a heartbeat is outstanding despite unavailable receipt storage', async () => {
  ...
  expect(h.sent.at(-1).request.type).toBe('heartbeat')
  const heartbeat = h.sent.at(-1).request.requestId, stopped = h.c.stopTransmit()
  expect(h.sent.at(-1).request.type).toBe('stopTransmit')
  expect(h.sent.at(-1).request.transmitEpoch).toBe(h.s.transmitEpoch)
  expect(h.c.getSnapshot().busy).toBe(true)
  await expect(h.c.stopTransmit()).rejects.toThrow('remoteBusy')
  h.c.receive({ type: 'operationResponse', requestId: h.sent.at(-1).request.requestId, value: { stop: 'accepted' } })
...
it.each(['timeout', 'disconnect', 'refused'])('does not retry or claim RF stopped after %s', async cause => {
  const h = client(), stopped = h.c.stopTransmit(), requestId = h.sent.at(-1).request.requestId
