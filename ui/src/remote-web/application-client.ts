...
import { PARKS_COMMAND, CONFIGURATION_COMMAND, configurationCollection, NAVIGATION_COMMAND, navigationCollection, SSTV_IMAGE_COMMAND, APRS_COMMAND, JS8_CONTEXT_COMMAND, FIELD_DAY_COMMAND, OTA_COMMAND, MEMORIES_COMMAND, DXPEDITIONS_COMMAND, INSIGHTS_COMMAND, QUERY_COMMAND, RECALL_COMMAND, insightCollection } from './application-query-protocol'
...
export class ApplicationClient implements ApplicationTransport {
  ...
  open(): void {
    ...
    this.send(JSON.stringify(this.serviceVersion >= 2 ? { type: 'applicationHello', version: this.serviceVersion } : { type: 'applicationHello' }))
  }
  disconnected(): void {
  ...
  invoke<T>(command: string, args?: Record<string, unknown>): Promise<T> {
    if (this.phase === 'ready' && this.version >= 15 && command === PARKS_COMMAND) {
      if (args?.collection !== 'parks') return Promise.reject(new Error('applicationUnsupported'))
    ...
    if (this.phase === 'ready' && this.version >= 7 && command === DXPEDITIONS_COMMAND) {
      if (args?.collection !== 'dxpeditions') return Promise.reject(new Error('applicationUnsupported'))
      return this.query.read(args, 7) as Promise<T>
    }
    if (this.phase === 'ready' && this.version >= 6 && command === INSIGHTS_COMMAND) {
      if (!insightCollection(args?.collection)) return Promise.reject(new Error('applicationUnsupported'))
      return this.query.read(args!, 6) as Promise<T>
    }
