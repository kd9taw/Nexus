...
export class ApplicationClient implements ApplicationTransport {
  ...
  invoke<T>(command: string, args?: Record<string, unknown>): Promise<T> {
    if (this.phase === 'ready' && this.version >= 16 && command === POUNCE_COMMAND) {
      if (args?.collection !== 'pounce') return Promise.reject(new Error('applicationUnsupported'))
      return this.query.read(args, 16) as Promise<T>
    }
