  ...
  receiveStation(raw: unknown): void {
    ...
    try {
      ...
      if ('value' in response && 'phase' in response.value && (p.operationVersion ?? 2) < 4) {
        ...
        if (response.value.controls) response.value.controls.capabilities = response.value.controls.capabilities.filter(c => c !== 'ftOperate' && c !== 'ftCall' && c !== 'ftExchange' && c !== 'ftMessages')
      }
