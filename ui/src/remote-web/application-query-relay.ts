...
export type QueryCheckpoint = { pending: Pending | null; windowAt: number; count: number }
...
export class ApplicationQueryRelay {
  ...
  sync(station: Peer | null, observers: { sessionId: string; peer: Peer }[], now: number): void {
  ...
  add(id: string, now: number): void {
    ...
    if (peer) this.browsers.set(id, { peer, pending: null, count: 0, windowAt: now })
  }
  checkpoint(id: string): QueryCheckpoint | undefined {
    const b = this.browsers.get(id)
    if (b) return { pending: b.pending && { ...b.pending }, count: b.count, windowAt: b.windowAt }
  }
  restore(id: string, saved: QueryCheckpoint): void {
    const peer = this.peers.get(id)
    if (!peer || !Number.isSafeInteger(saved.count) || saved.count < 0 || saved.count > 16 ||
  ...
  receiveBrowser(id: string, message: Record<string, unknown>, now: number): void {
    ...
    try {
      ...
      const request = queryRequest(message)
