...
export function controlVersion(action: StationAction): 2 | 3 {
  return ['radio.frequency', 'radio.mode', 'radio.tier', 'radio.workspace', 'radio.select', 'amplifier.followBand', 'decoder.js8Speed', 'decoder.msk144Period', 'decoder.depth', 'receiver.rxOffset'].includes(action.action) ? 3 : 2
}
