...
export function controlVersion(action: StationAction): 2 | 3 | 4 {
  if (action.action === 'ft.setting' || action.action === 'qso.logCurrent' || action.action === 'qso.confirm' || action.action === 'qso.discard' || action.action === 'ft.message' || action.action === 'ft.exchange' || action.action === 'ft.call' || action.action === 'ft.cq' || action.action === 'ft.txEnabled') return 4
