...
describe('every rig in the catalog can actually be loaded by the Hamlib we ship', () => {
  ...
  it('⭐ the fixture COVERS the catalog — a rig added since the last regeneration fails here', () => {
    // THE FT-890 HOLE, and it is why this test reads the catalog instead of the fixture. The
    // fixture is generated BY HAND and nothing forces a re-run, so a rig added afterwards is
    // simply absent — and every other guard in this file iterates the FIXTURE, which makes an
    // absent rig invisible to all of them, the FX-4 guard included. The FT-890 sat outside them
    // for exactly that reason: a one-rate 4,800 radio nothing could see, left on the 38,400
    // default. A fixture cannot vouch for its own completeness; only the catalog can.
    const here = (rel: string) => fileURLToPath(new URL(rel, import.meta.url))
    const catalog = readFileSync(here('../../../crates/tempo-audio/src/rigmodels.rs'), 'utf8')
    const inCatalog = [...catalog.matchAll(/^\s*\(\s*(\d+)\s*,/gm)].map((m) => Number(m[1]))
    // The parse is this test's own positive control: a pattern that silently matched nothing
    // would leave `unaccounted` empty and the assertion below would pass on no evidence at all.
    expect(inCatalog.length).toBeGreaterThan(100)
    const accountedFor = new Set<number>([
      ...hamlibSerialSpeeds.rigs.map((r) => r.model),
      ...hamlibSerialSpeeds.missing,
      ...hamlibSerialSpeeds.needsDaemon,
      ...hamlibSerialSpeeds.noSerial,
    ])
    const unaccounted = [...new Set(inCatalog)].filter((m) => !accountedFor.has(m)).sort((a, b) => a - b)
    expect({ unaccounted }).toEqual({ unaccounted: [] })
  })

  it('the serial-less bucket is a NAMED set, not a dumping ground', () => {
    // Five backends declare no serial port and belong here: Dummy, FLRig, TRX-Manager and the
    // two Flex entries. A real radio landing in this bucket is the defect — the FT-890 did, and
    // was absorbed without a sound because the bucket was an unrecorded `continue`. Pinning the
    // membership is what turns the next one into a failure instead of a silence.
    expect([...hamlibSerialSpeeds.noSerial].sort((a, b) => a - b)).toEqual([1, 4, 5, 2036, 23005])
  })
