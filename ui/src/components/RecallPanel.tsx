...
interface Props {
  ...
  /** ⭐ Whether THIS contest's dupe rule names the band at all (`FieldDayStatus.dupeRule.byBand`).
   *
   *  It decides only what the badge SAYS, never whether it lights — `contestDupe` has always
   *  applied exactly the components the rule names. Sweepstakes names none but the callsign
   *  (rule 2.2: a station is worked once, regardless of band), so the badge correctly lights
   *  for a 40 m contact while the operator is on 20 m — and the tooltip, which interpolated the
   *  CURRENT band, then sent him to look at 20 m, where by definition there was nothing. In the
   *  one shipped contest where a dupe cannot be on the band he is looking at.
   *
   *  ⚠️ Default TRUE, matching `contestDupe`'s own `LEGACY_TRIPLE`: a station too old to send a
   *  rule meant `(call, band, mode class)`, and a host outside a contest passes nothing and gets
   *  the sentence this card has always shown. */
  contestDupeByBand?: boolean
}
          ...
          {contestDupe === 'own' && (
            <span
              ...
              /* FOUR literal call sites rather than one with a computed key: the i18n
                 extractor reads keys statically, and a ternary INSIDE `t()` hides them
                 from it — they read as orphan entries nobody references, and the
                 unreadable-call-site ratchet ticks up. Both guards caught it.
                 The `…AnyBand` pair takes no {{band}} at all, which is the fix: under a
                 ruleset that works a station once regardless of band, the band the
                 operator is sitting on is the one place the contact is NOT. */
              title={
                contestDupeByBand
                  ? contestLogsDupes
                  ...
                  : contestLogsDupes
                    ? t('recall.contestDupe.titleLoggedAnyBand')
                    : t('recall.contestDupe.titleAnyBand')
              }
            >
