...
export function useStationCapability(capability: ControlCapability): boolean {
  ...
  return local || !!(available && (!['ftOperate', 'ftCall', 'ftExchange'].includes(capability) || (client?.operationVersion ?? 0) >= 4) && (!expanded || (client?.operationVersion ?? 0) >= 3) && view?.fresh && view.connected && view.requestReady !== false && !view.unresolved && !view.controlPending &&
