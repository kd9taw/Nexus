 ...
 * It is `SingleFlightLatch` (singleFlight.ts, which the IPC pollers share since #335) with the
 * row's patience and the row's wording.
 *
 * ⚠️ WHY THIS IS A LATCH AND NOT TWO BOOLEANS (field reports, 2026-08-17: "the waterfall
 ...
 *
 * The escape needs a GENERATION COUNTER, not just a flag reset: once the watchdog gives a call
 * up, that call may still resolve later, and it must then neither draw (its row is stale and the
 * history ring must stay 1:1 with what was blitted) nor clear the latch (which by then belongs to
 * a live fetch). `SingleFlightLatch` explains both halves.
 */
export class RowFetchLatch extends SingleFlightLatch {
  constructor(label: string, limitMs: number = ROW_FETCH_STUCK_MS) {
    super(label, limitMs, 'spectrum row fetch')
  }
}
