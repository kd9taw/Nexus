#!/usr/bin/env bash
# Measure the contest entry strip in a REAL layout engine and print the JSON.
#
# ⚠️ jsdom lays out nothing — `getBoundingClientRect` is 0×0 there, so the width budget
# in spec §9 cannot be checked from vitest. This runs the harness page under headless
# Chrome, which does lay out, and prints what `entry-strip.html`'s `run()` measured.
#
...
if [ -z "$chrome" ]; then
  ...
  echo "Install one, or open entry-strip.html in a browser and call run()." >&2
  exit 2
fi
...
dom=$("$chrome" --headless=new --disable-gpu --no-sandbox \
  ...
  --dump-dom "file://$here/entry-strip.html" 2>/dev/null)
status=$?
if [ $status -ne 0 ]; then
...
if [ -z "$json" ]; then
  echo "the harness page produced no measurements — did entry-strip.html throw?" >&2
  exit 1
fi
