...
fn remote_spot_requires_v3_and_readback_before_native_handoff_without_replay() {
    // Native CW commands plain `CW` on EVERY band, and Remote matches it. The band rule
    // that made 160/80/40 m ask for `CWR` was reversed on 2026-09-11: there is no such CW
    // convention, and `CWR` means the LOWER side on a Yaesu and the UPPER side on a default
    // Icom, so no band rule expressed in CW/CWR could be right for both. The preference
    // survives as the station-wide `cw_reverse`, default OFF. Phone's LSB below 10 MHz is
    // untouched — that convention is real.
