...
fn restart_live_feeds(
    ...
    std::thread::spawn(move || {
        ...
        let (cluster_active, cluster_hosts, mycall, mygrid, opening_regional) = match engine.lock()
        {
            Ok(eng) => {
                let st = eng.settings();
                (
...
mod tests {
    use super::{
        ...
        is_complete_lotw_body, iss_pass_from_tles, parse_sstv_mode, profile_dir_name,
        sanitize_profile, AssistanceEvent, AssistanceSourceState,
    };
