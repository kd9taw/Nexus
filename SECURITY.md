...
Tempo is a small, volunteer, open-source project — a chat-first HF
text-messaging app for off-grid/preparedness hams. We take security
seriously within the limits of what a one-person hobby project can promise.
Please read this whole page before reporting; it sets honest expectations.
...
Tempo 0.1.x is **beta**. Published Windows binaries are cross-compiled and
the modem has been validated in **simulation only** — not yet on-air. There is
no separate "stable" branch; fixes land on the 0.1.x line.
...
- The Tempo version (e.g. `0.1.0`) and OS (Tempo's primary target is Windows).
...
Tempo is a **desktop** application. Its trust boundary is the local machine and
the operator's own LAN. The surfaces most relevant to a security report:

- **WSJT-X-compatible UDP listener** — Tempo speaks the WSJT-X UDP protocol
  (magic `0xADBCCBDA`, schema 3). The conventional sink is `127.0.0.1:2237`.
  Besides outbound Status/Decode datagrams, Tempo's server can accept inbound
...
- **PSK Reporter outbound** — Tempo can send spot reports as outbound UDP to
...
- **`rigctld` control port (localhost TCP)** — for CAT/PTT control Tempo
...
- **Tempo spawns `rigctld`** — when CAT PTT is selected, Tempo runs the bundled
...
- The inherent properties of amateur radio: HF is an open, unauthenticated,
  unencrypted medium by law and by design. Tempo does not (and must not, under
  Part 97) encrypt over-the-air traffic. Spoofed or replayed RF frames are a
  property of the band, not a Tempo vulnerability.
...
Tempo is licensed under **GPL-3.0-or-later** (see `COPYING`). As stated there,
...
*Tempo — <https://github.com/kd9taw/tempo> — Seth McCallister (KD9TAW)
