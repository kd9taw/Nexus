...
- **Only a contact logged with the park counts.** HUNT, double-clicking the park on the Connect map, and **Work** on the Needed board all tag the next contact with that activator for you — and so does starting the contact from the Call Roster, Band Activity or the station list, when the activator is spotted at exactly one park right now. Spotted at two (a summit that is also a park), that tags nothing; pick the one you are working with HUNT first. A contact logged without the park reference hides nothing — that is the safe direction, but it can look as if the chip is not working. Any band or mode counts; the activator's portable suffix (`K1ABC/P`) does not matter.
