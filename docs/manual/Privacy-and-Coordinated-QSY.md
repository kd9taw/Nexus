...
> **Read this first — the honest ceiling.** On the amateur bands there is **no such thing as a private conversation.** Tempo's Coordinated QSY makes a QSO a little *harder to find and follow* for a casual listener. It is **not** encryption, **not** a secret hopping pattern, and **not** privacy. A capable listener with a wide receiver can still find you, decode every word, and follow you. Tempo is designed this way **on purpose**, to stay legal.
...
- **§97.309** requires data codes to be **publicly documented.** Tempo's waveforms and this QSY scheme are open-source (GPLv3) — that's the documentation.
...
So the only lever Tempo can legally pull is **low-probability-of-intercept (LPI)** — making the QSO *harder to stumble onto*, not *harder to read*. The realistic benefit is against **casual** listeners: someone spinning the dial, a club member parked on the calling frequency, an HT scanner. Against a software-defined radio recording the whole band, **none of these techniques do anything** — and Tempo says so plainly in the app.
...
- **Synced by the clock.** Both stations share Tempo's UTC slot clock, so the directive names an absolute slot to move on and both retune on the **same** T/R boundary — they land together.
...
Both operators must have the feature enabled for a move to happen. If your partner doesn't run Tempo (or has it off), they'll simply see the directive as a plain-text message and stay put — so nothing moves unless you've both opted in.

---

## What Tempo deliberately does **not** do
...
*See also: [Frequency Plan](Frequency-Plan.md) · [Operating Guide](Operating-Guide.md) · [FAQ](FAQ.md)*
