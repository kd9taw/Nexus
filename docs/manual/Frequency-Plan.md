...
Tempo uses a **new waveform** (FT1 / DX1), so it must **not** transmit on the established FT8 / FT4 / JS8 / WSPR / PSK watering holes — it would QRM them, and they'd QRM it. So Tempo ships its **own** calling frequencies, chosen to sit clear of every existing narrowband convention while staying inside the operator's legal sub-band.

> **These are proposed, editable defaults — not regulatory channels.** They're a starting point to coordinate with the digital community. In the app you can pick any band with one tap **or type any dial frequency manually** (top bar and Settings). **Confirm your local band plan and operate within your license privileges.** For the full rationale and verification notes, see the developer reference: [`docs/FREQUENCIES.md`](https://github.com/kd9taw/tempo/blob/main/docs/FREQUENCIES.md).

---

## Why these frequencies

- **Off the watering holes.** New modes that squatted on existing FT8/FT4/JS8 frequencies caused real friction; community buy-in matters. Tempo deliberately sits clear.
- **HF — the "upper shoulder of the digital cluster."** On each band the dense FT8 → JT65 → JS8 → FT4 block runs from the FT8 dial up a few kHz. Tempo sits just **above** that block and **below** the fragile WSPR window, inside the IARU narrowband-modes segment — the same way JS8 was sited relative to FT8.
...
| Band | Tempo freq (MHz) | Mode | Sits clear of | Notes |
|------|------------------|------|---------------|-------|
...
- **Proposed + editable.** Coordinate with the community before treating any of these as "the" Tempo frequency; the app lets you override any dial.
...
- **Cramped bands.** On 17 m and 12 m the usable gap is only a few kHz, so DX1's ~50 Hz signal fits better than a ~150 Hz FT1 signal there. See [Tiers FT1 vs DX1](Tiers-FT1-vs-DX1.md).
...
## Changing frequency in Tempo

- **Band selector** (top bar and Settings) — a grouped HF / VHF / UHF channel list. One tap jumps the rig to that band's Tempo frequency and mode.
- **Manual entry** — type any dial frequency (MHz) and pick **USB** or **FM**; Tempo retunes the rig live (via `rigctld`/CAT) and labels the band automatically.
