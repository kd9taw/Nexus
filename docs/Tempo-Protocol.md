...
>
> **Honesty banner, up front.** Two different things are true here and they are easy to conflate. **The waveforms work on the air:** the first over-the-air TempoFast decode was on 2026-07-21 and the first two-station Tempo QSO (KD9TAW and N9UM, 6 m) completed on 2026-07-26. **The numbers are not from the air:** every sensitivity figure in this document comes from AWGN and Rayleigh-fading sweeps in the project's test harnesses (`ft1_test_standalone`, `dx1_test`, re-validated in the Rust test suite). What is still owed is a proper characterisation of decode rate against signal level on real paths, and that is the open gate. Where the implementation does not yet expose a feature, that is stated plainly. Read accordingly.
...
- **The waveforms close real links.** The first over-the-air TempoFast decode was on 2026-07-21, and the first two-station Tempo QSO (KD9TAW and N9UM, 6 m) completed on 2026-07-26. The biggest risk any new waveform carries, whether it works outside a simulator at all, is retired.
- **The sensitivity figures are still bench numbers.** Both the TempoFast (~−15 dB AWGN) and TempoDeep (~−18.6 dB AWGN, ~3.7 dB fading penalty) thresholds come from AWGN and Rayleigh-fading sweeps (`ft1_test_standalone` / `dx1_test`, re-validated in the Rust suite). Real propagation, QRM, and operator behaviour are not in the simulation, so a proper characterisation of decode rate against signal level on real paths is the open gate.
...
- **IR-HARQ is live; its gains are bench-measured.** Joint iterative turbo combining of RV0/RV1/RV2 is wired end-to-end and **on by default**; the QSO sequencer drives RV escalation, `Decode.rv` reports how many RVs were combined, and RV detection runs >99% accurate to −11 dB. The measured gains in §6 (combiner +1.3 dB AWGN / +3.2 dB fading at 3-TX; ~+2.5 dB threshold shift and ~2× completion through the full pipeline) are **bench measurements from simulation and the Windows cross-build**, not on-air results.
...
TempoFast is a coherent 4-CPM turbo modem on a 4-second conversational cycle, sharing the WSJT-X 77-bit payload and LDPC(174,91) FEC with the modes you already know — so the *messages* are familiar even though the *waveform* is not. Its distinguishing bets are **coherence** (more information per second, at the cost of fading fragility), the **4 s cycle** (conversation, not slideshow), and **IR-HARQ** (redundancy that *accumulates* across retransmissions via joint turbo combining — unique among amateur weak-signal text modes, now live and on by default, measured at ~+2.5 dB / ~2× completion in simulation). TempoDeep backstops the fading fragility with a non-coherent, fading-robust tier on the same payload. TempoFast does not out-sensitize FT8; it trades ~6 dB of raw single-shot reach against FT8 (~2.5 dB against FT4) for speed and an incremental-redundancy path. Both tiers have closed real links on the air, including a completed two-station QSO, and every number above still awaits the on-air decode-rate characterisation that is the project's next gating step.

---

*Grounding: the waveform/FEC/sync constants come from the modem source — `libtempo/include/libtempo.h`, `libtempo/tempofast_cabi.f90`, `libtempo/tempodeep/*.f90`, `crates/ft1-sys/src/lib.rs`, `crates/ft1/src/lib.rs`, and `crates/tempo-core/src/timing.rs`. The IR-HARQ schedule and the design-analysis throughput/threshold figures in §6 are from the project's Phase-1 IR-HARQ protocol-design analysis (a separate research document); the measured combiner/pipeline gains are from the live HARQ validation harness. All §6 figures are simulation- and Windows-cross-build-validated, not on-air measurements. See also `docs/ARCHITECTURE.md` and the operator manual (`docs/manual/`).*

---

*Author: KD9TAW. License: GPL-3.0. Repository: [sourceforge.net/projects/nexus-ham-radio](https://sourceforge.net/projects/nexus-ham-radio) · Manual: `docs/manual/` (see `Tempo-Chat.md`, `Architecture-and-Protocol.md`, `Roadmap.md`).*
