...
> the TempoFast/TempoDeep protocols are simulation-validated, not yet on-air-proven. The
...
- **TempoFast & TempoDeep — new protocols, honest numbers.** A chat layer on TempoFast, a
  4-second-cycle weak-signal mode with IR-HARQ retransmission combining, plus
  TempoDeep, a fading-resilient robust tier. TempoFast trades ~6 dB of raw single-shot
  sensitivity against FT8 (~−15 vs ~−21 dB, simulated) for a nearly 4× faster
  cycle — every TempoFast/TempoDeep figure is simulation-validated, not on-air-proven.
...
Bug reports and TempoFast/TempoDeep on-air decode reports are the most valuable thing you
