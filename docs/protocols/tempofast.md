...
TempoFast runs a **4-second transmit/receive cycle** instead of FT8's 15 seconds (FT4 is
...
take to send one FT8 frame. That is the point of TempoFast: **keyboard chat at
...
TempoFast does **not** invent a new way to say "CQ." It reuses the **WSJT-X 77-bit
...
- The Chat, QSO, and Field-Day message forms ride identically on TempoFast, TempoDeep, and the
...
This is the part of TempoFast with no analog in FT8, FT4, or JS8, so it is worth
explaining plainly.
...
TempoFast borrows a trick from the cellular world called **IR-HARQ** (incremental-
...
exists. And if a retransmission goes unacknowledged, an TempoFast station falls back to
...
| What | Figure | Basis |
|---|---|---|
| TempoFast standalone threshold | **≈ −15 dB** AWGN | simulation |
| FT8, for comparison | ≈ −21 dB | commonly-cited operational figure |
| Gap TempoFast gives up to FT8 | **≈ 6 dB** (and ≈ 2.5 dB vs FT4) | simulation vs. cited FT8 |
...
**TempoFast is not more sensitive than FT8. It is faster.**

FT8's ~−21 dB single-shot threshold makes it the most sensitive mode here; TempoFast's
~−15 dB puts it roughly where FT4 sits. TempoFast gives up about **6 dB** of raw reach to
...
FT8 (≈3.5 dB for half the cycle) — TempoFast just pushes the lever further and adds
...
- band and dial frequency, and which tier (TempoFast vs TempoDeep),
