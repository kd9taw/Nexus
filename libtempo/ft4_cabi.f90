...
! tempofast_cabi.f90. Unlike FT8, the WSJT-X FT4 decoder (ft4_decode::decode) is a
! clean, self-contained OO decoder with a callback (no nzhsym/a7/shmem), so we
! drive it directly via a collector callback exactly like tempofast_cabi does for FT1.
!
! Underlying Fortran:
...
module ft4_cabi
  ...
  ! Interop result struct. Layout MUST match ft4_decode_t in libtempo.h
