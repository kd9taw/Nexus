! Tempo: C ABI wrappers for the standalone FT1 modem (libtempo).
!
...
!   genft1            (gen_tempofast.f90)            - message -> 99 channel symbols
!   gen_ft1wave       (gen_tempofastwave.f90)       - symbols -> real audio waveform
!   ft1_downsample    (ft1_downsample.f90)    - audio -> complex baseband
!   turbo_decode_ft1  (turbo_decode_tempofast.f90)  - baseband -> 91 message bits
!
...
module tempofast_cabi
  ...
  ! match ft1_decode_t in libtempo.h (see header for the exact byte layout).
  ! ------------------------------------------------------------------------
  ...
  ! Layout MUST match dx1_decode_t in libtempo.h:
...
contains
  ...
  !   dx1_encode_msg (gen_tempodeepwave.f90) - text -> 77 bits -> 174-bit codeword
  !   gen_dx1wave    (gen_tempodeepwave.f90) - codeword -> chirp sync + 8-FSK audio
