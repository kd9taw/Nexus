...
! MODIFIED FOR NEXUS (KD9TAW, 2026-07-27): the .pcoeff training dump is removed.
! On a converged training run this wrote a timestamped
! "<DXCALL>_YYMMDD_HHMMSS.pcoeff" file into datadir - several hundred lines of
! phase-fit coefficients and per-sample residuals, for offline analysis of the
! MSK144 phase equalizer. Nothing in Nexus reads it, and datadir does not exist in
! a headless build; keeping it would have created files wherever Nexus happened to
! be started from, the same defect q65_write_red had.
!
! ONLY the I/O is excised. The state updates that follow (training_dxcall,
! currently_training, btrain, navg) are KEPT, so if training is ever enabled
! through the C ABI it still completes and latches correctly - it just does not
! dump coefficients to disk. Removing those too would have left training
! permanently "in progress".
!
! Unreachable as shipped in any case: msk144_cabi pins btrain=.false., and this
! block is gated on currently_training.
        training_dxcall(1:12)=' '
        currently_training=.false.
        btrain=.false.
        navg=0
