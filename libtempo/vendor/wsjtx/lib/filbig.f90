...
! MODIFIED FOR NEXUS (KD9TAW, 2026-07-27): the two fftwf_plan_with_nthreads calls
! are removed, leaving every plan single-threaded.
!
! NOT merely a dependency trim — the call was BROKEN in a headless build. nthreads
! comes from `common/patience/npatience,nthreads` (:29), and nothing in the
! vendored tree ever assigns that common block: it is declared here and in
! four2a.f90:32 and written nowhere. (msk144sync.f90:59 and msk40sync.f90:59 set a
! LOCAL nthreads for their OMP regions; different variable.) In WSJT-X the Qt side
! fills it from the GUI's FFTW-patience setting. Here it is BSS-zero, so the call
! was `fftwf_plan_with_nthreads(0)` — an invalid thread count, on every plan.
!
! Linking libfftw3f_threads (which does exist on this host) would therefore have
! made things WORSE, not better: it would have turned an unresolved symbol into a
! live threaded plan built with a zero thread count, and it would have added a
! threads-variant FFTW to the Windows cross-build's requirements for no benefit.
!
! Plans 2 and 3 were ALREADY single-threaded upstream — the original
! `call fftwf_plan_with_nthreads(1)` immediately below plan1 restored a count of 1
! before building them. So the only behavioural change is that plan1 (the
! nfft1-point r2c transform) is now single-threaded too, which is what a count of
! 0 could never have delivered anyway.
