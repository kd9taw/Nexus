...
! MODIFIED FOR NEXUS (KD9TAW, 2026-07-27): `use prog_args` removed — WSJT-X's
! program-argument module, written by jt9.f90's parser, which libtempo does not
! vendor. Its only use here was building paths for the file I/O excised below.
    use types
...
! MODIFIED FOR NEXUS: the tsil.3q caller-history READ is removed, and this is a
! correctness fix as much as a de-coupling. tsil.3q is written by q65.f90 (see the
! matching excision there) and read back here, so with two radio chains in one
! process chain A's caller history reaches chain B THROUGH THE FILESYSTEM — a
! contamination path no per-chain in-memory context swap can close. Identical shape
! to fst4_decode's fst4w_calls.txt, found the same way.
!
! nhist2=0 is what an absent file already produced upstream (the `end=2` branch), so
! the contest AP path simply starts with no remembered callers. Restoring it means
! feeding the history through the C ABI, not reopening a file. Unreachable today in
! any case: q65_cabi pins ncontest=0 and this block is gated on ncontest.eq.1.
       nhist2=0
    endif
