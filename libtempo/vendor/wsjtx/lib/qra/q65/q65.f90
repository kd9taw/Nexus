...
subroutine q65_hist2(nfreq,msg0,callers,nhist2)
  ...
  use types
! MODIFIED FOR NEXUS (KD9TAW, 2026-07-27): `use prog_args` removed; see the
! matching note in q65_decode.f90.
...
! MODIFIED FOR NEXUS: the tsil.3q caller-history WRITE is removed. This is the
! producing half of the cross-chain path described in q65_decode.f90: it persists
! THIS decoder's `callers` table to disk on hard-coded unit 24, where another radio
! chain in the same process reads it back. The in-memory `callers` table is still
! maintained and still used within a single decode; only the disk round-trip goes.
