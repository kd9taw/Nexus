...
//! round-trip through `libtempo` (Fortran + FFTW3f + C/C++) and prints PASS/FAIL.
//!
...
fn ft1_roundtrip() -> Result<String, String> {
    ...
    let mut buf = tempo_fast::frame_align(&wave);
    let mut rng = Awgn::new(12345);
    for s in buf.iter_mut() {
        *s = sig * *s + rng.gaussian();
    }

    let d = tempo_fast::decode_rt(&buf, F0, SNR_DB);
    if !d.ok() {
        return Err(format!(
...
fn dx1_roundtrip() -> Result<String, String> {
    let wave = tempo_fast::deep::encode_wave(MSG, tempo_fast::deep::F0, tempo_fast::SAMPLE_RATE);
    if wave.len() != tempo_fast::deep::frame_len() {
        return Err(format!(
    ...
    match tempo_fast::deep::decode(&window, tempo_fast::deep::F0, tempo_fast::SAMPLE_RATE) {
...
fn main() {
    ...
    if ok {
        println!("\nALL PASS — libtempo (Fortran + FFTW3f + C/C++) works on this target.");
    } else {
