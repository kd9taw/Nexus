//! Raw FFI bindings to `libft1` — the standalone FT1 4-CPM turbo modem.
//!
//! See `tempo/libft1/include/libft1.h` for the authoritative ABI documentation.
...
/// Serializes ALL access to the non-thread-safe `libft1` modem.
///
...
/// FT1, FT8, FT4 and DX1** (they all link this one `libft1`). Every safe wrapper
...
/// `libft1.h` (68 bytes, 4-byte aligned; `#[repr(C)]` reproduces the 2-byte pad
/// after `message`).
#[repr(C)]
...
/// matches `dx1_decode_t` in `libft1.h` (52 bytes, 4-byte aligned; the 2-byte
...
/// `ft8_decode_t` / `ft4_decode_t` in `libft1.h` (64 bytes, 4-byte aligned;
...
mod tests {
    ...
    /// `libft1.h` / `ft1_cabi.f90`. A drift here would silently corrupt every
    ...
    /// `libft1.h` / `ft1_cabi.f90` (52 bytes; 2-byte tail pad after message).
    #[test]
    ...
    /// to `ft8_decode_t` / `ft4_decode_t` in `libft1.h` / `ft8_cabi.f90` /
