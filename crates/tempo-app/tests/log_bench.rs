...
// is linked into this binary through tempo-core, so its C API is here to call without naming the
// crate that wraps it.
extern "C" {
...
fn the_log_path_bench() {
    ...
    println!(
        "LOG_BENCH load average {load:?} on {cpus} CPUs: {}",
        if busy {
